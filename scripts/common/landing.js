document.addEventListener('DOMContentLoaded', () => {
  ...
  function checkSearchButton() {
    const departureCity = document.getElementById('departure-city-input').value.trim();
  ...
  function renderFlights(flights) {
    ...
    flights.forEach(flight => {
      ...
      flightElement.innerHTML = `
        ...
        <p class="duration">${Math.round((new Date(flight.arrival_time) - new Date(flight.departure_time)) / (1000 * 60 * 60))} hours</p>
  ...
  function filterFlights() {
    ...
    const arrivalCity = document.getElementById('arrival-city-input').value.trim().toLowerCase();
    const passengers = document.getElementById('select-passengers').value.trim();
    ...
    const filteredFlights = flightsData.filter(flight => {
        ...
        const matchesDepartureCity = !departureCity || flightDepartureCity.includes(departureCity);
        const matchesArrivalCity = !arrivalCity || flightArrivalCity.includes(arrivalCity);
        const matchesPassengers = !passengers || flight.available_seats >= parseInt(passengers);
